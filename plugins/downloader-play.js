...
let handler = async (m, { conn, text, usedPrefix }) => {
  ...
  let buttons = [{ buttonText: { displayText: 'Audio' }, buttonId:  `.yta ${url}` }, { buttonText: { displayText: 'Video' }, buttonId:  `.ytv ${url}` }]
  let msg = await conn.sendMessage(m.chat, { image: { url: thumbnail }, caption: `*Title:* ${title}\n*Published:* ${ago}\n*Duration:* ${duration}\n*Views:* ${views}\n*Description:* ${description}\n*Url:* ${url}`, footer: '_Please select the button below_', buttons }, { quoted: m })
